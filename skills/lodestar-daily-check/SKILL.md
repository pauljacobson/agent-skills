...
4. **A tracked Linear item with a stale gap.** Per `config.md` § Linear and the
   `goal-mapping.md` "Tracked Linear issues" map. Thresholds are **per-row**:
   surface when `stale.age_days ≥ row.staleness_days` (7 days for TSCODE-406, 14
   for the WoW project, defaults otherwise) AND the item wasn't surfaced within
   its `reminder_days` window (`nudges/log.jsonl`, keyed by the row's `ref`,
   e.g. `TSCODE-406` → 3 days). Independent of the parent goal's journal signal.
   Cite the open-gap count + the oldest gap's date; offer a small concrete step
   (e.g. "want to slot 20 minutes to clear one, or push it to Todoist?").
